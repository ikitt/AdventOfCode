...
/* Explication de la logique pour la partie 2parce que ça peut servir. (lien du puzzle : https://adventofcode.com/2021/day/8)
 * Les digit 1, 4, 7 et 8 ont chacun un nombre unique de segments => on les détermine donc en premier
 * |_> 3 est le seul digit à 5 segment qui inclut tout les segment de 1
 * |_> 9 est le seul digit à 6 segment qui inclut tout les segment de 3
 * |_> 0 est le seul digit à 6 segment qui inclut tout les segment de 1 et qui est != de 9
 * |_> 6 est le seul digit à 6 segment restant
 * |_> 5 est le seul digit à 5 segment dont tous sont  inclus dans 6
 * |_> 2 est le seul digit qui reste.
 */
...
void Y21Day08::computFirstResult()
{
    int res = 0;
    QVector<int> uniqueSize = {2,3,4,7};
    for(QString line : _input)
    {
        QStringList output = line.split("|", QString::SkipEmptyParts)[1].split(" ", QString::SkipEmptyParts);
        for(QString outDigit : output)
        {
            if(uniqueSize.contains(outDigit.size()))
                res++;
        }
...
void Y21Day08::computSecondResult()
{
    int res = 0;
    _digitEquivalent = {};
    _digitEquivalent.resize(10);

    QVector<int> uniqueSize = {2,3,4,7};
    for(QString line : _input)
    {
        QStringList unknownDigit = line.split("|", QString::SkipEmptyParts)[0].split(" ", QString::SkipEmptyParts);
        QStringList output = line.split("|", QString::SkipEmptyParts)[1].split(" ", QString::SkipEmptyParts);

        get1478(unknownDigit);
        get3(unknownDigit);
        get9(unknownDigit);
        get0(unknownDigit);
        get6(unknownDigit);
...
//        fprintf(stdout, "Out is %i\r\n", getOutInteger(output));

        res += getOutInteger(output);

    }

    fprintf(stdout, "Res for second part is %i\r\n", res);
    fflush(stdout);
}


void Y21Day08::printKnown()
{
    fprintf(stdout, "Known are : ");
    for(int idx = 0 ; idx < _digitEquivalent.size() ; idx ++)
    {
        if(!(_digitEquivalent[idx].isEmpty() || _digitEquivalent[idx].isNull()))
            fprintf(stdout, " %i = %s ;", idx, _digitEquivalent[idx].toStdString().c_str());
    }
    fprintf(stdout, "\r\n");
    fflush(stdout);
}

void Y21Day08::get1478(QStringList& unknownDigit)
{
    for(int idx = 0 ; idx <unknownDigit.size() ; idx ++)
    {
        QString digit = unknownDigit[idx];
        for(int deterministicDigit : {1,4,7,8})
        {
            if(digit.size() == _nbrSegments[deterministicDigit])
            {
                _digitEquivalent[deterministicDigit] = digit;
                unknownDigit.removeAt(idx);
                idx--;
                continue;
            }
        }
...
void Y21Day08::get3(QStringList& unknownDigit)
{
    for(int idx = 0 ; idx <unknownDigit.size() ; idx ++)
    {
        QString digit = unknownDigit[idx];
        if((digit.size() == _nbrSegments[3]) && firstIncludeSecond(digit, _digitEquivalent[1]))
        {
            _digitEquivalent[3] = digit;
            unknownDigit.removeAt(idx);
            break;
        }
    }
...
void Y21Day08::get9(QStringList& unknownDigit)
{
    for(int idx = 0 ; idx <unknownDigit.size() ; idx ++)
    {
        QString digit = unknownDigit[idx];
        if((digit.size() == _nbrSegments[9]) && firstIncludeSecond(digit, _digitEquivalent[3]))
        {
            _digitEquivalent[9] = digit;
            unknownDigit.removeAt(idx);
            break;
        }
    }
...
void Y21Day08::get0(QStringList& unknownDigit)
{
    for(int idx = 0 ; idx <unknownDigit.size() ; idx ++)
    {
        QString digit = unknownDigit[idx];
        if((digit.size() == _nbrSegments[0]) && firstIncludeSecond(digit, _digitEquivalent[1]))
        {
            _digitEquivalent[0] = digit;
            unknownDigit.removeAt(idx);
            break;
        }
    }
...
void Y21Day08::get6(QStringList& unknownDigit)
{
    for(int idx = 0 ; idx <unknownDigit.size() ; idx ++)
    {
        QString digit = unknownDigit[idx];
        if(digit.size() == _nbrSegments[6])
        {
            _digitEquivalent[6] = digit;
            unknownDigit.removeAt(idx);
            break;
        }
    }
}
void Y21Day08::get5(QStringList& unknownDigit)
{
    for(int idx = 0 ; idx <unknownDigit.size() ; idx ++)
    {
        QString digit = unknownDigit[idx];
        if((digit.size() == _nbrSegments[5]) && firstIncludeSecond(_digitEquivalent[6], digit))
        {
            _digitEquivalent[5] = digit;
            unknownDigit.removeAt(idx);
            break;
        }
    }
}
void Y21Day08::get2(QStringList& unknownDigit)
{
    if(unknownDigit.size() != 1)
        throw std::domain_error("two many unknwon digit");
    else
        _digitEquivalent[2] = unknownDigit[0];
}

bool Y21Day08::firstIncludeSecond(QString first, QString second)
{
    bool isIncluded = true;
    for(QChar segment : second)
    {
        if(! first.contains(segment))
        {
            isIncluded = false;
            break;
        }
...
static void getOutInteger();
bool Y21Day08::isEquivalent(QString first, QString second)
{
    bool isEqui = true;
...
int Y21Day08::getOutInteger(QStringList output)
{
    int res = 0;
    int pow = 1000;
    for(QString digit : output)
    {
        for(int idx = 0 ; idx < _digitEquivalent.size() ; idx++)
        {
            if(isEquivalent(_digitEquivalent[idx], digit))
            {
                res += idx*pow;
                pow = pow/10;
                break;
...
QVector<QString> Y21Day08::_digitEquivalent = {};

const QVector<int> Y21Day08::_nbrSegments = {6,2,5,5,4,5,6,3,7,6}; //Number of segment per digit

//exemple input =>
/*Res first part = 26
///*Res second part = 61229
const QVector<QString> Y21Day08::_input = {
