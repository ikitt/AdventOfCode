...
unsigned long long int Y20Day14::computFirstResult()
{
    currentInputIdx = 0;

    QVector<unsigned long long int> memory(getMaxMemIdx()+1, 0);

    for(QString line : _input)
    {
        QRegularExpressionMatch matchMem = memRegExp.match(line);
        QRegularExpressionMatch matchMask = maskRegExp.match(line);
        if(matchMem.hasMatch() && matchMem.lastCapturedIndex() >= 1)
        {
            unsigned long long int idx = matchMem.captured(1).toUInt();
            unsigned long long int value = matchMem.captured(2).toUInt();
            value = applyMaskV1On(value);
            fprintf(stdout, "apply %llu to mem[%llu] !\r\n", value, idx);
            fflush(stdout);

            memory[idx] = value;
        }
        else if(matchMask.hasMatch() && matchMask.lastCapturedIndex() >= 1)
        {
            currentMask = matchMask.captured(1);
        }
        else
        {
            fprintf(stdout, "unrecognized line bis");
            fflush(stdout);
            throw "unrecognized line bis";
        }
    }

    unsigned long long int sumMem = 0;
    unsigned long long int previousSumMem = 0;
    for(unsigned long long int value : memory)
    {
        if(value != 0)
        {
            fprintf(stdout, "Add %llu to %llu !\r\n", value, sumMem);
            fflush(stdout);
        }
        ...
        if(previousSumMem > sumMem) //Check for overflow
        {
            fprintf(stdout, "that's too big bro !\r\n");
            fflush(stdout);
        }
        previousSumMem = sumMem;
    }
    return  sumMem;
}


void testLambda( void(*billy)(int))
{
    int georege = 4;
    billy(georege);
}
...
unsigned long long int Y20Day14::computSecondResult()
{
    currentInputIdx = 0;

//    QVector<unsigned long long int> memory(std::pow(2,36), 0);
    QMap<unsigned long long, unsigned long long> momery;

//    QString boy;
//    memory[12]= 14;
//    auto boby = [&memory](int arg){memory[arg] = 12/*boy = QString::number(arg)*/;};
//    testLambda( boby);

    for(QString line : _input)
    {
        fprintf(stdout, "bad print with %s\r\n", line.toStdString().c_str());
        fflush(stdout);
        ...
        if(matchMem.hasMatch() && matchMem.lastCapturedIndex() >= 1)
        {
            unsigned long long int idx = matchMem.captured(1).toUInt();
            unsigned long long int value = matchMem.captured(2).toUInt();
            QString idxStr = applyMaskV2On(idx);

            std::function<void (unsigned long long)> mylambda =[&](unsigned long long possibleIdx){momery[possibleIdx] = value;
                /*fprintf(stdout, "Apply %llu to %llu\r\n", value, possibleIdx);
                fflush(stdout);*/};

            forEachPossibility(idxStr, mylambda);
        }
        else if(matchMask.hasMatch() && matchMask.lastCapturedIndex() >= 1)
        {
            currentMask = matchMask.captured(1);
        ...
        {
            fprintf(stdout, "unrecognized line");
            fflush(stdout);
    ...
    unsigned long long int sumMem = 0;
    unsigned long long int previousSumMem = 0;
    for(unsigned long long int key : momery.keys())
    {
        unsigned long long int value = momery[key];
        if(value != 0)
        {
//            fprintf(stdout, "Add mem[%llu]=%llu to %llu !\r\n", key, value, sumMem);
//            fflush(stdout);
        }
...
//void Y20Day14::forEachPossibility(QString idxStr, void(funLambda)(unsigned long long int))
//void Y20Day14::forEachPossibility(QString &idxStr, std::function<(unsigned long long)> funLambda)
//void Y20Day14::forEachPossibility(QString &idxStr, std::function<void (unsigned long long)> &funLambda){}
void Y20Day14::forEachPossibility(QString& idxStr, std::function<void(unsigned long long int)>& funLambda)
{
    QStringList possibleIdx = possibleStrValue(idxStr);
    for (QString Idx : possibleIdx)
    {
        (funLambda)(stringToInt(Idx));
    }
}


QStringList Y20Day14::possibleStrValue(QString input)
{
    QStringList res = {};
    ...
    {
        input[firstX] = '0';
        res.append(possibleStrValue(input));
        input[firstX] = '1';
        res.append(possibleStrValue(input));
        return res;
    }
...
void Y20Day14::myTest(std::function<void (unsigned long long)> &marco)
{
    unsigned long long param = 12;
    marco(param);
}



void Y20Day14::readNextInput()
{
    //TRVD
    unsigned long long toUpdate;
    std::function<void (unsigned long long)> mylambda =[&](unsigned long long paparam){toUpdate = paparam;};
    myTest(mylambda);
            //            auto lambada = [&](unsigned long long int possibleIdx){memory[possibleIdx] = value;};
}

unsigned long long int Y20Day14::getMaxMemIdx()
{
    unsigned long long int currentMax = 0;
    for(QString line : _input)
    {
        QRegularExpressionMatch match = memRegExp.match(line);
        if(match.hasMatch() && match.lastCapturedIndex() >= 1)
        {
            currentMax = std::max(currentMax, (unsigned long long int)(match.captured(1).toUInt()));
        }
    }


    fprintf(stdout, "Max of _intput is %llu.\r\n", currentMax);
    fflush(stdout);
    return currentMax;
}


unsigned long long int Y20Day14::applyMaskV1On(unsigned long long int value)
{
    QString valueBinStr = intToString(value);

//    fprintf(stdout, "Input %llu ", value);
//    fflush(stdout);
//    fprintf(stdout, "to binary give %s.\r\n", valueBinStr.toStdString().c_str());
//    fflush(stdout);

    for(int binIdx = 0 ; binIdx < 36; binIdx++)
    {
        if(currentMask[binIdx] != "X")
            valueBinStr[binIdx] = currentMask[binIdx];
    }

    unsigned long long int retVal = stringToInt(valueBinStr);

//    fprintf(stdout, "with mask                    %s\r\napply to binary give         %s.\r\n", currentMask.toStdString().c_str(), valueBinStr.toStdString().c_str());
//    fprintf(stdout, "Which is %llu.\r\n", retVal);
//    fflush(stdout);
...
QString Y20Day14::applyMaskV2On(unsigned long long int idx)
{
    QString idxBinStr = intToString(idx);
    ...
    for(int binIdx = 0 ; binIdx < 36; binIdx++)
    {
        if(currentMask[binIdx] == "0")
            idxBinStr[binIdx] = idxBinStr[binIdx];
        else if(currentMask[binIdx] == "1")
            idxBinStr[binIdx] = currentMask[binIdx];
        else if(currentMask[binIdx] == "X")
        {
            idxBinStr[binIdx] = currentMask[binIdx];
        }
        else
        {
            fprintf(stdout, "Bad value in mask");
            fflush(stdout);
            throw "Bad value in mask";
        }
    }
...
unsigned long long int Y20Day14::stringToInt(QString intput)
{
    int strIdx = 0;
    unsigned long long int retVal = 0;
    for(int powExp = 35 ; powExp >= 0 ; powExp--)
    {
        if(intput[strIdx] == '1')
            retVal += std::pow(2, powExp);
        strIdx++;
    }
    return retVal;
}
...
QString Y20Day14::intToString(unsigned long long int intput)
{
    QString valueBinStr = QString(36, '0');

    int strIdx = 0;
    for(int powExp = 35 ; powExp >= 0 ; powExp--)
    {
        if(std::pow(2,powExp) <= intput)
        {
            valueBinStr[strIdx] = '1';
            intput -= std::pow(2,powExp);
        }
        strIdx++;
    }
...
unsigned long long int Y20Day14::currentInputIdx = 0;
QString Y20Day14::currentMask = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
QRegularExpression Y20Day14::maskRegExp("mask = (\\w{36})");
QRegularExpression Y20Day14::memRegExp("mem\\[(\\d+)\\] = (\\d+)\\s*$");



 /* //cut input
const QVector<QString> Y20Day14::_input = {
...
"mem[23059] = 0                             "}; // */
 ...
 /* //given example; result should be 208
const QVector<QString> Y20Day14::_input = {
...
// /* //true input     2 190 765 291 748 is too high; 2 173 858 456 958
const QVector<QString> Y20Day14::_input = {
"mask = 1X01XXX001101X00001100X1010X10101101",
"mem[62085] = 231745                        ",
"mem[14249] = 1252796                       ",
