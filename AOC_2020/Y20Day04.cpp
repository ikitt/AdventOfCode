...
int Y20Day04::computFirstResult()
{
    int validPassCpt = 0;

    QList<QString> passportList = {};
    QString currentPasseport = "";
    ...
    for(QString line : _input)
    {
        ...
        if(simpLine.isEmpty())
        {
            passportList.append(currentPasseport);
            currentPasseport = "";
        }
        else
        {
            currentPasseport += (" " + simpLine);
        }
    }
    passportList.append(currentPasseport);

    for(QString pass : passportList)
    {
        validPassCpt += (passIsValid_partial(pass) ? 1 : 0);
    }


    fprintf(stdout, "There is %i partialy valid passeport\r\n", validPassCpt);
    fflush(stdout);

    return validPassCpt;
}

bool Y20Day04::passIsValid_partial(QString pass)
{
    bool isOk = true;

//    fprintf(stdout, "Check passport %s\r\n", pass.toStdString().c_str());
//    fflush(stdout);

    isOk &= containString(pass,"byr");
    isOk &= containString(pass,"iyr");
    isOk &= containString(pass,"eyr");
    isOk &= containString(pass,"hgt");
    isOk &= containString(pass,"hcl");
    isOk &= containString(pass,"ecl");
    isOk &= containString(pass,"pid");

    return isOk;
}

bool Y20Day04::containString(QString input, QString shouldBeIn)
{
    if (input.contains(shouldBeIn))
        return true;
    else {
//        fprintf(stdout, "Passport %s do not contain %s\r\n", input.toStdString().c_str(), shouldBeIn.toStdString().c_str());
//        fflush(stdout);
        return false;
    }
}

int Y20Day04::computSecondResult()
{
    int validPassCpt = 0;

    QList<QString> passportStrList = {};
    QString currentPasseport = "";
    ...
    for(QString line : _input)
    {
        ...
        if(simpLine.isEmpty())
        {
            passportStrList.append(currentPasseport);
            currentPasseport = "";
        }
        else
        {
    ...
    QList<Passport> passportList;
    for(QString pass : passportStrList)
    {
        Passport passport = fillPassport(passportList, pass);
        validPassCpt += (passIsValid(passport) ? 1 : 0);
//        validPassCpt += (passIsValid_partial(pass) ? 1 : 0);
    }


    fprintf(stdout, "There is %i filled passeport\r\n", passportList.size());
    fprintf(stdout, "There is %i fully valid passeport\r\n", validPassCpt);
    fflush(stdout);

    return validPassCpt;
}

Y20Day04::Passport Y20Day04::fillPassport(QList<Passport>& list, QString passStr)
{
    Passport pass;
    pass.completeStr = passStr;
    QStringList itemList = passStr.split(" ", QString::SkipEmptyParts);
    for(QString item : itemList)
    {
        if(item.contains("byr"))
            pass.byr = item.split(":")[1];
        else if(item.contains("iyr"))
            pass.iyr = item.split(":")[1];
        else if(item.contains("eyr"))
            pass.eyr = item.split(":")[1];
        else if(item.contains("hgt"))
            pass.hgt = item.split(":")[1];
        else if(item.contains("hcl"))
            pass.hcl = item.split(":")[1];
        else if(item.contains("ecl"))
            pass.ecl = item.split(":")[1];
        else if(item.contains("pid"))
            pass.pid = item.split(":")[1];
        else if(item.contains("cid"))
            pass.cid = item.split(":")[1];
        else
        {
            fprintf(stdout, "Unknown item '%s'\r\n", item.toStdString().c_str());
            fflush(stdout);
        }
    }
...
bool Y20Day04::passIsValid(Passport& pass)
{
    bool passOk = true;
    passOk &= validate_byr(pass);
    passOk &= validate_iyr(pass);
    passOk &= validate_eyr(pass);
    passOk &= validate_hgt(pass);
    passOk &= validate_hcl(pass);
    passOk &= validate_ecl(pass);
    passOk &= validate_pid(pass);
...
bool Y20Day04::validate_byr(Passport pass)
{
    bool isValid = false;
    QRegularExpression regExp("(\\d{4})");

    QRegularExpressionMatch match;
    match = regExp.match(pass.byr);
    if(match.hasMatch() && (match.lastCapturedIndex() >= 1) && (pass.byr.size() == 4))
    {
        int year = pass.byr.toUInt();
...
//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm byr (%s) for '%s'\r\n", pass.byr.toStdString().c_str(), pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
bool Y20Day04::validate_iyr(Passport pass)
{
    bool isValid = false;
    QRegularExpression regExp("(\\d{4})");

    QRegularExpressionMatch match;
    match = regExp.match(pass.iyr);
    if(match.hasMatch() && (match.lastCapturedIndex() >= 1) && (pass.iyr.size() == 4))
    {
        int year = pass.iyr.toUInt();
...
//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm iyr (%s) for '%s'\r\n", pass.iyr.toStdString().c_str(), pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
bool Y20Day04::validate_eyr(Passport pass)
{
    bool isValid = false;
    QRegularExpression regExp("(\\d{4})");

    QRegularExpressionMatch match;
    match = regExp.match(pass.eyr);
    if(match.hasMatch() && (match.lastCapturedIndex() >= 1) && (pass.eyr.size() == 4))
    {
        int year = pass.eyr.toUInt();
...
//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm eyr (%s) for '%s'\r\n", pass.eyr.toStdString().c_str(), pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
bool Y20Day04::validate_hgt(Passport pass)
{
    bool isValid = false;
    QRegularExpression regExp("(\\d{2,3})([cmin]{2})");

    QRegularExpressionMatch match;
    match = regExp.match(pass.hgt);
    if(match.hasMatch() && (match.lastCapturedIndex() >= 2) && (pass.hgt.size() >= 4))
    {
        int size = match.captured(1).toUInt();
        if((match.captured(2) == "cm") && (size >= 150) && (size<= 193))
            isValid = true;
        else if((match.captured(2) == "in") && (size >= 59) && (size<= 76))
            isValid = true;
    }

//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm hgt (%s), match = \"%s\"for '%s'\r\n", pass.hgt.toStdString().c_str(), match.captured(0).toStdString().c_str(),pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
bool Y20Day04::validate_hcl(Passport pass)
{
    bool isValid = false;
    QRegularExpression regExp("#([0-9a-f]{6})");

    QRegularExpressionMatch match;
    match = regExp.match(pass.hcl);
    if(match.hasMatch() && (match.lastCapturedIndex() >= 1) && (pass.hcl.size() == 7))
    {
        isValid = true;
    }
...
//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm hcl (%s) for '%s'\r\n", pass.hcl.toStdString().c_str(), pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
bool Y20Day04::validate_ecl(Passport pass)
{
    bool isValid = false;
    QStringList okColor = {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"};
    if(okColor.contains(pass.ecl))
        isValid = true;

//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm ecl (%s) for '%s'\r\n", pass.ecl.toStdString().c_str(), pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
bool Y20Day04::validate_pid(Passport pass)
{
    bool isValid = false;
    QRegularExpression regExp("(\\d{9})");

    QRegularExpressionMatch match;
    match = regExp.match(pass.pid);
    if(match.hasMatch() && (match.lastCapturedIndex() >= 1) && (pass.pid.size() == 9))
    {
        isValid = true;
    }
...
//    if(!isValid)
//    {
//        fprintf(stdout, "Pbm pid (%s) for '%s'\r\n", pass.pid.toStdString().c_str(), pass.completeStr.toStdString().c_str());
//        fflush(stdout);
//    }
...
const QStringList Y20Day04::_input = {
"ecl:amb                                                                                                                 ",
"pid:690616023                                                                                                           ",
