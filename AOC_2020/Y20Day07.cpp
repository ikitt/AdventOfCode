...
Bag::Bag(QString color)
    :_color(color)
{

}

void Bag::printDirectContent()
{
    fprintf(stdout, "Bag with color %s contain: \r\n", _color.toStdString().c_str());
    for(QString color : _directContent.keys())
    {
        fprintf(stdout, "    %i bag(s) of color -%s-\r\n", _directContent[color], color.toStdString().c_str());
    }
    fflush(stdout);
}

void Bag::printIndirectContent()
{
    fprintf(stdout, "Bag with color %s contain: \r\n", _color.toStdString().c_str());
    for(QString color : _indirectContent.keys())
    {
        fprintf(stdout, "    %i bag(s) of color -%s-\r\n", _indirectContent[color], color.toStdString().c_str());
    }
    fflush(stdout);
}

void Bag::fillIndirectContent(QMap<QString, Bag*> bagsList)
{
    for(QString outColor: bagsList.keys())
    {
//        for(QString color : bagsList[outColor]->_directContent.keys())
//        {
            recursiveAdd(outColor, outColor, 1, bagsList);
//        }
    }
}

void Bag::recursiveAdd(QString baseColor, QString currentColor, int factor, QMap<QString, Bag*> bagsList)
{
//    fprintf(stdout, "recursiveAdd(%s, %s %i)%s\r\n",baseColor.toStdString().c_str() , currentColor.toStdString().c_str(),factor);
    fflush(stdout);
    for(QString color : bagsList[currentColor]->_directContent.keys())
    {
        int currentFactor = factor * bagsList[currentColor]->_directContent[color];
        bagsList[baseColor]->addToColor(color, currentFactor);
        if(bagsList.keys().contains(color))
            recursiveAdd(baseColor, color, currentFactor, bagsList);
    }

}

void Bag::addToColor(QString color, int number)
{
    if(_indirectContent.find(color) != _indirectContent.end())
        _indirectContent[color] = _indirectContent[color] + number;
    else
        _indirectContent[color] = number;
//    fprintf(stdout, "Add %i %s in %s\r\n",number, color.toStdString().c_str(), _color.toStdString().c_str());
    fflush(stdout);
}
...
int Y20Day07::computFirstResult()
{
    computeRules();
    return 0;
}
...
int Y20Day07::computSecondResult()
{
    return 0;
}

bool Y20Day07::computeRules()
{
    QRegularExpression bagsRE("bags?\\.?");
    QMap<QString, Bag*> bagsList = {};

    for(QString line : _input)
    {
        QString simpLine = line.simplified();

        QStringList firstSplit = simpLine.split(" contain ");
        QString outsideColor = firstSplit[0].remove(bagsRE).simplified();

        Bag* outsideBag = new Bag(outsideColor);


        QStringList insideList = firstSplit[1].split(", ");
        for(QString insideCol: insideList)
        {
            insideCol.remove(bagsRE);
            QRegularExpression regExp("(?<insideNumber>\\d+) (?<insideColor>\\w+ \\w+)");
            QRegularExpressionMatch match = regExp.match(insideCol);

            fprintf(stdout, "In %s\r\n",insideCol.toStdString().c_str());
            if(match.hasMatch())
            {
                int number = match.captured("insideNumber").toInt();
                QString color = match.captured("insideColor");
//                fprintf(stdout, "Get %i of color -%s- in -%s- bag \r\n", number, color.toStdString().c_str(), outsideColor.toStdString().c_str());
                outsideBag->_directContent[color] = number;
            }
...
//        outsideBag->printDirectContent();
        bagsList[outsideBag->_color]=outsideBag;
    }
    Bag::fillIndirectContent(bagsList);

    fprintf(stdout, "12\r\n");
    fflush(stdout);
    for(QString color: bagsList.keys())
    {
//        bagsList[color]->printIndirectContent();
    }
    fprintf(stdout, "13\r\n");
    fflush(stdout);
    ...
    //Solus for first part
//    int bagWithShinyGoldCount = 0;
//    for(Bag* bag: bagsList)
//    {
//        if(bag->_indirectContent.find("shiny gold") != bag->_indirectContent.end())
//            bagWithShinyGoldCount++;
//    }

//    fprintf(stdout, "There is %i bags with shiny gold\r\n", bagWithShinyGoldCount);
//    fflush(stdout);

    int totalNumber = 0;
    for(int number: bagsList["shiny gold"]->_indirectContent)
    {
        totalNumber +=number;
    }
        fprintf(stdout, "There is %i bags in a shiny gold\r\n", totalNumber);
        fflush(stdout);
...
//const QStringList Y20Day07::_input = {
...
//    "    bright lavender bags contain 10 pale bronze bag.                                                                    "
//};
