...
int Y20Day06::computFirstResult()
{
    int sumQuestion = 0;
    ...
    {
        QList<QString> answeredYes = {};
        for(QString letter: group)
        {
            if(!answeredYes.contains(letter))
                answeredYes.push_back(letter);
        }

        sumQuestion += answeredYes.size();
    }


    fprintf(stdout, "The sum of yes question per group is  %i\r\n", sumQuestion);
    fflush(stdout);

    return sumQuestion;
}

int Y20Day06::computSecondResult()
{
    int sumQuestion = 0;
    ...
    for(QString line : _input)
    {
        ...
        {
            currentGroup.append(simpLine);
        }
    }
    groupList.append(currentGroup);

    QList<QChar> firstAnswer = {};
    QList<QChar> removeList = {};
    for(QStringList group : groupList)
    {
        firstAnswer.clear();
        removeList.clear();
        for(QChar letter : group[0])
        {
            firstAnswer.append(letter);

//            fprintf(stdout, "Appen '%c' to first Answer\r\n", letter.toLatin1());
        }
        for(QString person : group)
        {
//            fprintf(stdout, "Next\r\n");
            for(QChar letter : firstAnswer)
            {
                if(!person.contains(letter))
                {
                    removeList.append(letter);
//                    fprintf(stdout, "Remove all '%c' to first Answer\r\n", letter.toLatin1());
                }
            }
//            fprintf(stdout, "Get this ");
            for(QChar letter : firstAnswer);
//                fprintf(stdout, "%c", letter.toLatin1());
//            fprintf(stdout, "\r\n");
        }

        for(QChar letter : removeList)
        {
            firstAnswer.removeAll(letter);
        }

//        fprintf(stdout, "Add %i to sum\r\n", firstAnswer.size());
        sumQuestion += firstAnswer.size();
    }
...
/*
const QStringList Y20Day06::_input = {
"abc",
"   ",
"a  ",
"b  ",
